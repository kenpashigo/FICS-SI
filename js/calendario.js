...
function moveCalendario() {   
  ...
  for(var i=0;i<42;i++) {      
    ...
    if(campoIndex == diaDaSemana) {
      ...
      if(diaValor == dday && month == change) { campoCalendario[i].style = "background: #3ed162;";} 
      ...
      for(var j=0;j<resultados.length;j++){            
        ...
        if(diaValor == resultados[j].childNodes[1].innerText) {
          ...
          campoCalendario[i].style = "color: red; font-size: 20px; font-weight: 700;";          
        }
      }
      ...
      if(campoIndex == 0 || campoIndex == 6) {campoCalendario[i].style = "color: #315f3c;";}      
...
function calendario_change(a){     
   ...
   xmlhttp.open("GET","./paginas/calenreturn.php?sem="+a,true);
   xmlhttp.send();   
}
